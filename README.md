...
2. **Linear Acceleration Data**  
   ...
   - This produces an estimate of **position vectors** in $x$, $y$, and $z$.
...
- **Position Panel:**  
  Displays the estimated Position values for $x$, $y$, and $z$, as well as the total Position magnitude.
...
To set up the environment with `venv` and process the data that is inside the `./data` folder and create the html in `./track` folder, simply run:
